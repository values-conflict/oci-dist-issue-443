...
[`edbe27f`](https://github.com/opencontainers/distribution-spec/commit/edbe27fcffdb28899fff98f81747f8b7980fd590) deleted `detail.md` entirely, with the rationale that it
contained non-conformant and extraneous information. However, a significant amount of
conformance-relevant protocol behavior was deleted along with it and was never restored.

The current `spec.md` is 921 lines. The pre-reorganization `spec.md` was 4,603 lines.

## Issues

Each file below documents one category of lost content. For each:

- **What was lost** — quoted original text
...
| File | Topic |
|------|-------|
| [blob-range-requests.md](blob-range-requests.md) | 206 Partial Content response format; 416 on blob fetch; Accept-Ranges detection |
| [accept-ranges-detection.md](accept-ranges-detection.md) | `Accept-Ranges: bytes` on HEAD enables capability detection before range GET |
...
| [rate-limiting-semantics.md](rate-limiting-semantics.md) | 429 SHOULD include `TOOMANYREQUESTS` body and `Retry-After` header |
...
| [link-header-format.md](link-header-format.md) | Pagination `Link` header requires angle brackets; absence means end of results; worked example |
...
| [caching-etag-guidance.md](caching-etag-guidance.md) | Blobs are immutable and SHOULD be aggressively cached; ETags on manifests enable conditional GET |
| [compliance-definition.md](compliance-definition.md) | Explicit definition of what "compliant" means (MUST violations = non-compliant; SHOULD violations are not) |

## Repository Commits Used for Evidence

Client and server citations are pinned to the following commits:
